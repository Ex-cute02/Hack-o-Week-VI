...
const WS_URL = `ws://localhost:${config.WS_PORT}`;
...
function createClient(userId: string, deviceId: string): void {
  ...
  ws.on("message", (data: WebSocket.RawData) => {
    ...
    if (msg.type === "ack" && messageCount === 0) {
      ...
      const interval = setInterval(() => {
        ...
        console.log(
          `${prefix} Sent telemetry #${messageCount}: HR=${payload.data.heart_rate}, Steps=${payload.data.steps}`,
        );
  ...
  ws.on("close", (code: number, reason: Buffer) => {
    console.log(
      `[${userId}] Disconnected (code=${code}, reason=${reason.toString()})`,
    );
  });
...
function createEdgeCaseClient(): void {
...
function createTestToken(userId: string): void {
  const token = generateJWT(userId);

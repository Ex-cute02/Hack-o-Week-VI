...
import { DEKCacheEntry } from "./types";
...
const SIMULATED_CMK = crypto.randomBytes(32);
const CMK_KEY_ID = "cmk-sim-" + crypto.randomBytes(4).toString("hex");
...
function checkCircuit(): void {
  if (circuitOpen) {
    if (Date.now() - circuitOpenedAt > CIRCUIT_RESET_MS) {
      ...
      logger.info("KMS circuit breaker reset (half-open)");
    } else {
...
function recordFailure(): void {
  consecutiveFailures++;
  if (consecutiveFailures >= config.KMS_MAX_FAILURES) {
    circuitOpen = true;
    circuitOpenedAt = Date.now();
    logger.error("KMS circuit breaker OPENED after consecutive failures", {
...
function generateDEK(): {
  ...
  const keyId = "dek-" + crypto.randomBytes(8).toString("hex");

  // Encrypt DEK with CMK (envelope encryption)
  const iv = crypto.randomBytes(12);
  const cipher = crypto.createCipheriv("aes-256-gcm", SIMULATED_CMK, iv);
  const encrypted = Buffer.concat([cipher.update(dek), cipher.final()]);
  const authTag = cipher.getAuthTag();

  const encryptedKey = Buffer.concat([iv, authTag, encrypted]).toString(
    "base64",
  );
...
export async function getDEK(
  ...
  try {
    ...
    dekCache.set(userId, {
      decryptedKey,
      keyId,
      expiresAt: Date.now() + config.DEK_CACHE_TTL_MS,
    });

    recordSuccess();
    logger.debug("DEK generated/cached for user", { userId, keyId });
...
export function encryptPayload(plaintext: string, key: Buffer): string {
  const timer = metrics.cryptoEncryptionDuration.startTimer();
  try {
    const iv = crypto.randomBytes(12);
 ...
 */
export function decryptPayload(cipherBase64: string, key: Buffer): string {
